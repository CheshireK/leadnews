package com.myapp.common.redis;
...
public class CacheService extends CachingConfigurerSupport {
     ...
     * @return 锁的值
     */
    public String tryLock(String lockName, long expire, TimeUnit unit){
        try {
            lockName = lockName + "_LOCK";
            String token = UUID.randomUUID().toString();
            Boolean setIfAbsent = stringRedisTemplate.opsForValue()
                    .setIfAbsent(lockName, token, expire, unit);
    ...
    public void unlock(String lockName, String token){
        lockName = lockName + "_LOCK";
        if (Objects.equals(stringRedisTemplate.opsForValue().get(lockName), token)){
